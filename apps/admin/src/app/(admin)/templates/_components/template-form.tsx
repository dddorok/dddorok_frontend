...
export function TemplateForm({
  ...
  const form = useForm<Template>({
    defaultValues: template || {
      ...
      measurementRuleId: initialRuleData?.measurementRuleId
        ? String(initialRuleData.measurementRuleId)
        : undefined,
    },
  });

  const [selectedToolType, setSelectedToolType] = useState<ToolType>(
    template?.toolType || "대바늘"
  );
  ...
  const [selectedRule, setSelectedRule] = useState<string | undefined>(
    template?.measurementRuleId
      ? String(template.measurementRuleId)
      : initialRuleData?.measurementRuleId
        ? String(initialRuleData.measurementRuleId)
        : undefined
  );
  ...
  useEffect(() => {
    if (initialRuleData?.measurementRuleId) {
      console.log(
        `Setting measurementRuleId in form: ${initialRuleData.measurementRuleId}`
      );
      ...
      if (initialRuleData.measurementItems) {
        form.setValue("measurementItems", initialRuleData.measurementItems);
        console.log(
          `Set ${initialRuleData.measurementItems.length} measurement items`
        );
      }
      // }
  ...
  useEffect(() => {
    ...
    const isKnittingTops =
      selectedToolType === "대바늘" && selectedCategory.level2 === 10; // 10 = 상의 카테고리
    ...
    const chartBasedPattern =
      selectedPatternType === "차트형" || selectedPatternType === "혼합형";
  ...
  useEffect(() => {
    if (selectedSleeveType && selectedCategory.level3) {
      const measurementRule = findMeasurementRule(
            ...
            {form.getValues("measurementRuleId") && selectedCategory.level3 && (
              <div>
                ...
                <div className="p-3 bg-gray-50 rounded-md border text-sm mt-1">
                  {selectedCategory.level1 &&
                    getCategoryById(selectedCategory.level1)?.name}
                  {selectedCategory.level2 &&
                    ` > ${getCategoryById(selectedCategory.level2)?.name}`}
                  {selectedCategory.level3 &&
                    ` > ${getCategoryById(selectedCategory.level3)?.name}`}
                </div>
            ...
            <CardContent className="space-y-6">
              ...
              {ruleValidationError && (
                <div className="p-4 border border-red-200 bg-red-50 rounded-md text-red-800">
                  ...
                  <p className="mt-1 text-sm pl-6">{ruleValidationError}</p>
                </div>
              )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select
                      onValueChange={(value) =>
                        field.onChange(value as NecklineType)
                      }
                      defaultValue={field.value}
                    >
                      <FormControl>
              ...
              <div className="mt-2">
                {form.getValues("measurementRuleId") ? (
                  <div className="p-4 border border-blue-100 bg-blue-50 rounded-md text-blue-800">
                    ...
                    {form.getValues("measurementItems")?.length &&
                      (form.getValues("measurementItems") as any).length >
                        0 && (
                        <div className="mt-2 pl-6 text-sm">
                          <strong>
                            설정된 항목 (
                            {form.getValues("measurementItems")?.length}개):
                          </strong>{" "}
                          {form
                            .getValues("measurementItems")
                            ?.map((itemId) => {
