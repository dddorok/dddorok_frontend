...
import { getSvgPreviewData } from "../new/utils/etc";
import { getGridLines } from "../new/utils/svgGrid";
// 임시
const scale = 1;
...
export function SvgPreview({
    ...
    for (let i = 1; i < coords.length; i++) {
      ...
      const base =
        typeof coords[i] === "number" && typeof coords[i - 1] === "number"
          ? coords[i]! - coords[i - 1]!
          : 0;
      result.push((prev ?? 0) + base * (intervals[i - 1] ?? 1));
    }
    return result;
  };
  ...
  const getScaledXY = (x: number, y: number) => {
    ...
    let newY = y;
    // x 구간 찾기
    if (rawXs.length > 1 && xs.length > 1) {
      let xIdx = rawXs.findIndex(
        (v, i) =>
          i < rawXs.length - 1 &&
          x >= (rawXs[i] as number) &&
          x <= (rawXs[i + 1] as number)
      );
      if (xIdx === -1) {
        xIdx = x < (rawXs[0] as number) ? 0 : rawXs.length - 2;
      }
      const x0 = rawXs[xIdx],
        x1 = rawXs[xIdx + 1];
      const x0p = xs[xIdx],
    ...
    // y 구간 찾기
    if (rawYs.length > 1 && ys.length > 1) {
      let yIdx = rawYs.findIndex(
        (v, i) =>
          i < rawYs.length - 1 &&
          y >= (rawYs[i] as number) &&
          y <= (rawYs[i + 1] as number)
      );
      if (yIdx === -1) {
        yIdx = y < (rawYs[0] as number) ? 0 : rawYs.length - 2;
      }
      const y0 = rawYs[yIdx],
        y1 = rawYs[yIdx + 1];
      const y0p = ys[yIdx],
  ...
  function transformPathD(d: string) {
    ...
    while ((match = commandRegex.exec(d)) !== null) {
      const [_, cmdRaw, paramsRaw] = match;
      if (!cmdRaw) continue;
      ...
      if (["M", "L", "T"].includes(cmdUpper)) {
        ...
        while (i < numbers.length) {
          ...
          if ([x1, y1, x2, y2, x, y].every((v) => typeof v === "number")) {
            const s1 = getScaledXY(x1 as number, y1 as number);
            const s2 = getScaledXY(x2 as number, y2 as number);
            const s = getScaledXY(x as number, y as number);
