...
export const measurementItemsBySection = () => {
  ...
  measurementItems.forEach((item) => {
    const itemGrouped = grouped[item.category];
    if (!itemGrouped) {
      grouped[item.category] = {};
      return;
    }

    if (!itemGrouped[item.section]) {
      itemGrouped[item.section] = [];
    }

    itemGrouped[item.section]?.push(item);
  });
...
export const NECKLINE_TYPES: NecklineType[] = [
  "라운드넥",
  "브이넥",
  "스퀘어넥",
];
...
export function findMeasurementRule(
  ...
  // 디버그용 로그 추가
  console.log(
    `Finding measurement rule for categoryId: ${categoryId}, sleeveType: ${sleeveType || "undefined"}`
  );
  console.log(
    "Available rules:",
    measurementRules.map((r) => ({
      id: r.id,
      categoryId: r.categoryId,
      sleeveType: r.sleeveType || "undefined",
      match:
        r.categoryId === categoryId &&
        (sleeveType ? r.sleeveType === sleeveType : !r.sleeveType),
    }))
  );

  if (sleeveType) {
    return measurementRules.find(
      (rule) => rule.categoryId === categoryId && rule.sleeveType === sleeveType
    );
  }
  return measurementRules.find(
    (rule) => rule.categoryId === categoryId && !rule.sleeveType
  );
}

// Function to get a measurement rule by ID
export function getMeasurementRuleById(
  ...
  // 대소문자 무시 비교
  rule = measurementRules.find(
    (r) => String(r.id).trim().toLowerCase() === normalizedId.toLowerCase()
  );
  if (rule) {
