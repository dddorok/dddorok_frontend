...
import { Info, List, Layers, Ban, XCircle } from "lucide-react";
...
export function MeasurementRuleList() {
  ...
  const handleDeleteRule = () => {
    if (deleteRuleId) {
      ...
      const updatedRules = measurementRules.filter(
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-amber-50 border-amber-200">
        ...
        <CardContent className="text-amber-700 space-y-2 text-sm">
          <p>
            <strong>기능 개요:</strong> 치수 규칙 설정은 의류 템플릿 생성의
            기본이 되는 측정 규칙을 정의하는 페이지입니다. 각 의류 유형과 소매
            유형별로 필요한 측정 항목들을 미리 정의하여 템플릿 생성 시
            활용합니다.
          </p>
          <p>
          ...
          <ul className="list-disc pl-5 space-y-1">
            ...
            <li>
              <strong>삭제 제한:</strong> 템플릿에서 사용 중인 치수 규칙은
              삭제할 수 없음 (사용 중인 템플릿이 있는 경우 삭제 전 경고 메시지
              표시)
            </li>
            <li>
          ...
          <p className="mt-3 bg-blue-50 p-2 rounded text-blue-800">
            ...
            <strong>UI 팁:</strong> 측정 항목 수를 클릭하면 상세 측정 항목
            목록을, 템플릿 수를 클릭하면 연결된 템플릿 목록을 확인할 수
            있습니다.
          </p>
        </CardContent>
      </Card>
      ...
      >
        ...
        <AlertDescription>
          ...
          <p className="mt-1">
            <strong>API 엔드포인트(예시):</strong> GET /api/measurement-rules
          </p>
        </AlertDescription>
      </Alert>
      ...
      {viewTemplatesRule && (
        <Dialog
          open={!!viewTemplatesRule}
          onOpenChange={(open) => !open && setViewTemplatesRule(null)}
        >
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>
            ...
            <div className="py-4">
              {(() => {
                const relatedTemplates = getTemplatesByRuleId(
                  viewTemplatesRule.id
                );
            ...
            <DialogFooter className="flex justify-between items-center">
              ...
              <Button
                variant="outline"
                onClick={() => setViewTemplatesRule(null)}
              >
                닫기
              </Button>
            </DialogFooter>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              '{ruleToDelete?.name}' 치수 규칙을 정말 삭제하시겠습니까? 이
              작업은 되돌릴 수 없습니다.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      <Dialog open={isErrorDialogOpen} onOpenChange={setIsErrorDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => setIsErrorDialogOpen(false)}>확인</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
